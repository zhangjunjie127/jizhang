...
import { Settings2, Wallet, ChevronRight, ShieldCheck, Sparkles, CircleHelp, MessageSquare, Info, Bell } from 'lucide-react';
import { APP_VERSION } from './account-services';
import { UserAvatar } from './user-avatar';
import './mine-page.css';

export function MinePage({ user, records, onSettings, onLedger, onAssistant, onUsage, onPrivacy, onService, onAccount, onMessages }) {
  ...
  return <main className="mine-home">
    ...
    <section className="mine-hero">
      <div className="mine-identity"><button type="button" className="mine-avatar-entry" aria-label="账号设置" title="账号设置" onClick={onAccount}><UserAvatar user={user} className="mine-avatar" /></button><div><h2>{user.name}</h2><p>{user.username} · 内测成员</p></div></div>
